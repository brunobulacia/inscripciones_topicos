...
export class EndpointsController {
  ...
  async create(@Body() createEndpointDto: CreateEndpointDto): Promise<EndpointResponseDto> {
  ...
  async getEndpointStats(): Promise<any> {
    return this.endpointsService.getEndpointStats();
  }

  @Get('queue-stats')
  @ApiOperation({ summary: 'Obtener estadísticas del sistema de encolado automático' })
  @ApiResponse({ status: 200, description: 'Estadísticas del encolado automático' })
  async getQueuingStats(): Promise<any> {
    return this.autoQueueService.getQueuingStats();
  }

  @Get('check-route')
  @ApiOperation({ summary: 'Verificar si una ruta está asignada a alguna cola' })
  @ApiQuery({ name: 'path', description: 'Ruta a verificar', example: '/api/carreras' })
  ...
  async findByColaId(@Param('colaId') colaId: string): Promise<EndpointResponseDto[]> {
    return this.endpointsService.findByColaId(colaId);
  }
  ...
  async findByColaNombre(@Param('nombreCola') nombreCola: string): Promise<EndpointResponseDto[]> {
    return this.endpointsService.findByColaNombre(nombreCola);
  }
  ...
  @ApiParam({ name: 'id', description: 'ID del endpoint' })
  @ApiResponse({ status: 204, description: 'Endpoint desactivado exitosamente' })
  @ApiResponse({ status: 404, description: 'Endpoint no encontrado' })
  ...
  async assignToCola(
    ...
    @Body() assignEndpointDto: AssignEndpointDto,
  ): Promise<EndpointResponseDto> {
    return this.endpointsService.assignToCola(colaId, assignEndpointDto);
  }
  ...
  @ApiParam({ name: 'colaId', description: 'ID de la cola' })
  @ApiResponse({ status: 204, description: 'Endpoint desasignado exitosamente' })
  @ApiResponse({ status: 404, description: 'Endpoint no encontrado en la cola' })
  async unassignFromCola(
    @Param('colaId') colaId: string,
    @Body() unassignEndpointDto: UnassignEndpointDto,
  ): Promise<void> {
    return this.endpointsService.unassignFromCola(colaId, unassignEndpointDto);
  }
}